...
\label{sec:org1a817af}

\begin{center}
\includegraphics[width=100%]{./img/herd.jpg}
\label{org5107e32}
\end{center}
...
\section{Hello World}
\label{sec:org3f942be}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/walloffame.jpg}
\caption{\label{fig:org0b13643}Wall of Fame. Katwijk, Netherlands.}
\end{figure}

\subsection{The Simplest Module}
\label{sec:org2090d7b}
...
You can use macros to indicate the license for your module. Some examples are "GPL", "GPL v2", "GPL and additional rights", "Dual BSD/GPL", "Dual MIT/GPL", "Dual MPL/GPL" and "Proprietary". They're defined within \textbf{linux/module.h}.
...
\begin{center}
\includegraphics[width=100%]{./img/phone.jpg}
\label{org680391b}
\end{center}

\subsection{Modules Spanning Multiple Files}
\label{sec:orgf5eeba5}
...
In this case, you need to restore the value of symbol \textbf{EXTRAVERSION} to -1.358. We suggest to keep a backup copy of the makefile used to compile your kernel available in \textbf{/lib/modules/5.17.3-1.358/build}. A simple \textbf{cp /lib/modules/`uname -r`/build/Makefile /usr/src/linux-`uname -r`} should suffice. Additionally, if you already started a kernel build with the previous (wrong) Makefile, you should also rerun make, or directly modify symbol UTS\_RELEASE in file \textbf{/usr/src/linux-5.17.3/include/linux/version.h} according to contents of file \textbf{/lib/modules/5.17.3/build/include/linux/version.h}, or overwrite the latter with the first.
...
\label{sec:orgf5272c4}

\subsection{How modules begin and end}
\label{sec:org187c095}
...
\label{sec:org8e68d94}
Memory management is a complicated subject and the majority of O'Reilly's "\emph{Understanding The Linux Kernel}" exclusively covers memory management! We're not setting out to be experts on memory managements, but we do need to know a couple of facts to even begin worrying about writing real modules.
...
By the way, I would like to point out that the above discussion is true for any operating system which uses a monolithic kernel. This isn't the same thing as \emph{"building all your modules into the kernel"}, although the idea is the same. There are things called microkernels which have modules which get their own codespace. The GNU Hurd and the Zircon kernel of Google Fuchsia are two examples of a microkernel.

\subsection{Device Drivers}
\label{sec:orge869443}
...
\label{sec:orgf98254b}
\subsection{The file\_operations Structure}
\label{sec:org0de8d3e}
The file\_operations structure is defined in \textbf{/usr/include/linux/fs.h}, and holds pointers to functions defined by the driver that perform operations on the device. Each field of the structure corresponds to the address of some function defined by the driver to handle a requested operation.
...
\label{sec:org0197c61}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/kreuzberg.jpg}
\caption{\label{fig:orgd1031e1}Wall painting near Maybachufer in Berlin-Kreuzberg, Germany}
\end{figure}
...
\label{sec:org011f98c}
As we have seen, writing a /proc file may be "complex". So to help people writting /proc file, there is an API named seq\_file that helps formating a /proc file for output. It's based on sequence, which is composed of 3 functions: start(), next(), and stop(). The seq\_file API starts a sequence when a user read the /proc file.
...
\label{sec:org8538ef6}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/luhansk.jpg}
\caption{\label{fig:orgb98436a}Lugansk, Ukraine}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/ximen.jpg}
\caption{\label{fig:org00b6fd3}Street art in Ximen/Taipei, Taiwan}
\end{figure}

\section{System Calls}
\label{sec:org996833a}
...
\label{sec:orgc302785}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/berlinwall.jpg}
\caption{\label{fig:org20d5353}Berlin Wall, 1989}
\end{figure}

\subsection{Sleep}
\label{sec:orgd380728}
...
\label{sec:org2a67a69}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/badass.jpg}
\caption{\label{fig:orge4b05c5}Graffiti at a former roof felt factory in Santalahti, Tampere, Finland}
\end{figure}
...
\label{sec:orgae35e7f}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/spin.jpg}
\caption{\label{fig:org45c6184}Graffiti from Spin}
\end{figure}
...
\label{sec:org0448e7b}
\subsection{Replacement}
\label{sec:org801fdda}
...
\label{sec:orgbbde862}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/bucharest.jpg}
\caption{\label{fig:org6e60176}Graffiti artist in Bucharest, Romania}
\end{figure}

\subsection{Interrupt Handlers}
\label{sec:org039460e}
...
\label{sec:org24cb135}
Typically you may want a random number to use as a password salt or capabilities token. Random numbers can be created first by assigning a generator and then using \textbf{crypto\_rng\_get\_bytes} to obtain however many random bytes that you need.
...
\label{sec:orgf48b485}
I probably don't have to warn you about this, but I figured I will anyway, just in case.

\section{Where To Go From Here?}
\label{sec:org3178a37}

\begin{figure}[htbp]
\centering
\includegraphics[width=100%]{./img/planet.jpg}
\caption{\label{fig:orgac385aa}Graffiti planet}
\end{figure}
